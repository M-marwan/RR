...
export default function Library() {
  useTelemetry("library");
  const { data } = useSWR<any[]>(
    "/api/intelligence/synthesis?job_type=newsletter_synthesis&limit=6",
    apiFetcher,
  );
    ...
    <div className="h-full overflow-y-auto p-6" style={{ background: "var(--rr-obsidian)" }}>
      ...
      <div className="grid grid-cols-2 gap-4">
        {SECTORS.map((sector) => {
          ...
          const summary = latest?.output_json?.summary;
          return (
            <div key={sector.key} className="rr-card p-5">
              ...
              {summary ? (
                <p className="text-xs leading-relaxed" style={{ color: "var(--rr-text)" }}>{summary}</p>
              ) : (
                <p className="text-xs" style={{ color: "var(--rr-subtle)" }}>
                  Awaiting paid source ingestion (Phase 2).
                </p>
              )}
            </div>
          );
